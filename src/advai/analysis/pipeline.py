...
def run_analysis_pipeline(
    ...
    # Setup separate activations CSV
    activations_csv_path = os.path.join(OUTPUTS_DIR, f"{RUN_TIMESTAMP}_{demos}", f"{RUN_TIMESTAMP}_{demos}_activations.csv")
    write_act_header = not os.path.exists(activations_csv_path) or os.stat(activations_csv_path).st_size == 0
    ...
    for idx, case in enumerate(tqdm(cases, desc="Processing cases")):
        ...
        for demo_combination in all_combinations:
            if demo_combination in case_demographic_combinations:
                ...
                sae_output = run_prompt(prompt, model, sae, clamping=False, clamp_features=None, clamp_value=0.0)
                ...
                for code in codes:
                    if "_@_" in code:
                        base, val_key = code.split("_@_", 1)
                        base_map = evidences.get(base, {})
                        question = base_map.get("question_en", base)
                        val_map = base_map.get("value_meaning", {}).get(val_key, {})
                        answer = val_map.get("en", val_key)
                        translation = f"{question}: {answer}"
                    else:
                        question = evidences.get(code, {}).get("question_en", code)
                        translation = f"{question}: Yes"
            ...
            # Write main results without activation columns
            main_fieldnames = FIELD_NAMES
            activ_fieldnames = [fn for fn in fieldnames if fn not in FIELD_NAMES]
            if prompt_outputs[group] is not None:
                ...
                # Filter and write main row
                main_row = {k: prompt_outputs[group].get(k) for k in FIELD_NAMES}
                with open(results_csv_path, "a", newline="") as main_csv:
                    main_writer = csv.DictWriter(main_csv, fieldnames=main_fieldnames)
                    if write_header:
                        main_writer.writeheader()
                        write_header = False
                    main_writer.writerow(main_row)
                # Filter and write activations row
                act_row = {"case_id": prompt_outputs[group].get("case_id")}
                for k in activ_fieldnames:
                    act_row[k] = prompt_outputs[group].get(k)
                with open(activations_csv_path, "a", newline="") as act_csv:
                    act_writer = csv.DictWriter(act_csv, fieldnames=["case_id"] + activ_fieldnames)
                    if write_act_header:
